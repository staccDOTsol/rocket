...
describe("rocket", () => {
  ...
  it("Is initialized!", async () => {
    ...
    , program.programId))[0]
    /*
    const tx4 = await program.methods.initialize(      (wen.toString())) 
    .accounts({
      game: game,
...
*/
    for (var a = 0; a < 10; a++){
    const someoneelse = (Keypair.fromSecretKey(bs58.decode(process.env.SOLANA_WALLET_PRIVATE_KEY)))
...
await delay(1000)
   ...
   const tx4 = await program.methods.play(      (wen.toString()),new BN( Math.floor(Math.random()*200000))) 
   .accounts({
     game: game,
     user: user,
   ...
   await delay(1000)
    await delay(1000)
    try {
    const tx3 = await program.methods.play(      (wen.toString()),new BN( Math.floor(Math.random()*200000))) 
    .accounts({
      game: game,
      user: user2,
      house: new PublicKey("JAcF8pPFvGrRCgbe2kAoEPXcgPVoLPNiC3Loz628C8sT"),
...
console.log(err)
    ...
    const tx6 = await program.methods.play(      (wen.toString()),new BN( Math.floor(Math.random()*200000))) 
    .accounts({
      game: game,
      user: user,
      house: new PublicKey("JAcF8pPFvGrRCgbe2kAoEPXcgPVoLPNiC3Loz628C8sT"),
  ...
  await delay(1000)
    ...
    const tx5 = await program.methods.play(      (wen.toString()),new BN( Math.floor(Math.random()*200000))) 
    .accounts({
      game: game,
      user: user,
      house: new PublicKey("JAcF8pPFvGrRCgbe2kAoEPXcgPVoLPNiC3Loz628C8sT"),
